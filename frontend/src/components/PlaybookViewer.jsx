...
export default function PlaybookViewer({ selectedItem, type }) {
  ...
  const fetchTopRisks = () => {
    ...
    graphApi.getTopRisks(10)
      .then(data => {
        setTopRisks(data);
        if (data && data.length > 0 && !selectedCve) {
  ...
  const generateRemediation = (cve) => {
    ...
    const payload = {
      cve_data: {
        cveId: cve.cveId || cve.cve_id || 'CVE-2026-1043',
  ...
  // Trigger MITRE Security Policy Generation
  const generatePolicy = (cve) => {
    if (!cve) return;
    ...
    const payload = {
      technique_data: {
        techniqueId: cve.cveId === 'CVE-2026-1043' ? 'T1190' : 'T1110',
        name: cve.cveId === 'CVE-2026-1043' ? 'Exploit Public-Facing Application' : 'Brute Force',
        tactic: cve.cveId === 'CVE-2026-1043' ? 'Initial Access' : 'Credential Access',
        capecId: cve.cveId === 'CVE-2026-1043' ? 'CAPEC-242' : 'CAPEC-112',
      }
    };

    playbooksApi.generateSecurityPolicy(payload)
      .then(res => {
    ...
    <div className="flex flex-col gap-8 w-full max-w-[92rem] mx-auto pb-12">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-4 flex flex-col gap-6">
          <div className="bg-[#0f172a]/95 backdrop-blur border border-slate-750 rounded-xl p-6 shadow-2xl flex flex-col gap-4">
            ...
            {risksLoading ? (
              <div className="py-20 flex flex-col items-center justify-center gap-3">
                ...
                <span className="text-[10px] text-slate-500 tracking-wider font-mono">LOADING RISKS...</span>
              </div>
            ) : (
                  ...
                  return (
                    ...
                    >
                      <div className="flex justify-between items-center">
                        <span className="text-sm font-bold font-mono text-slate-200">{risk.cveId}</span>
                        <span className={`text-[10px] font-black px-2 py-0.5 rounded ${
                          risk.riskLevel === 'CRITICAL' || risk.severity === 'CRITICAL' ? 'bg-red-500/20 text-red-400 border border-red-500/30' :
                          risk.riskLevel === 'HIGH' || risk.severity === 'HIGH' ? 'bg-amber-500/20 text-amber-400 border border-amber-500/30' :
                      ...
                      <div className="flex justify-between items-center mt-1 pt-2 border-t border-slate-800/60 text-[11px] font-mono text-slate-500">
                        <span className="flex items-center gap-1.5">
                          <Server size={12} className="text-cyan-400" />
                          {risk.assetName || risk.assetId || 'Asset_1'}
                        </span>
                        <span className="flex items-center gap-1 font-bold">
                          CVSS: <span className="text-slate-300">{risk.cvssScore || risk.cvss_score || 'N/A'}</span>
                        </span>
                      </div>
                    </motion.div>
                  );
        ...
        <div className="lg:col-span-8 flex flex-col gap-6">
          ...
          <div className="bg-slate-950 p-1.5 rounded-xl border border-slate-800 flex gap-2">
            <button
              ...
              className={`flex-1 py-3 text-sm font-bold rounded-lg uppercase tracking-wider transition-all duration-200 ${
            ...
            >
              MITRE Security Policy
            </button>
          </div>

          {/* Active Tab Panel */}
          <div className="bg-[#0f172a]/95 backdrop-blur border border-slate-750 rounded-xl p-7 shadow-2xl min-h-[600px]">
                    ...
                    {remediationPlaybook && (
                      <div className="flex gap-2">
                        <button
                          ...
                          className="px-3.5 py-2 bg-slate-900 border border-slate-800 hover:border-slate-700 text-slate-300 hover:text-white rounded-lg text-xs font-bold uppercase tracking-wider flex items-center gap-1.5 transition-all"
                        >
                        ...
                        <button
                          ...
                          className="px-3.5 py-2 bg-cyan-500/10 hover:bg-cyan-500/20 border border-cyan-500/20 hover:border-cyan-500/40 text-cyan-400 rounded-lg text-xs font-bold uppercase tracking-wider flex items-center gap-1.5 transition-all"
                        >
                          <Download size={14} />
                          Download
                        </button>
                  ...
                  {remLoading ? (
                    <div className="py-32 flex flex-col items-center justify-center gap-4">
                      ...
                      <div className="text-center">
                        <p className="text-xs text-slate-300 font-bold uppercase tracking-widest animate-pulse">
                    ...
                    <div className="flex flex-col gap-4">
                      {/* Section Accordions */}
                      {[
                        { key: 'executiveSummary', label: '1. Executive Summary', icon: ShieldCheck, color: 'text-cyan-400' },
                        { key: 'immediateActions', label: '2. Immediate Containment Actions', icon: AlertCircle, color: 'text-rose-400' },
                        { key: 'shortTermRemediation', label: '3. Short-Term Remediation', icon: Server, color: 'text-amber-400' },
                        { key: 'longTermHardening', label: '4. Long-Term Hardening', icon: Layers, color: 'text-blue-400' },
                        { key: 'verificationSteps', label: '5. Technical Verification Steps', icon: ShieldAlert, color: 'text-emerald-400' },
                        { key: 'rollbackPlan', label: '6. Disaster Recovery Rollback Plan', icon: Clock, color: 'text-purple-400' },
                        { key: 'complianceNotes', label: '7. Regulatory & Compliance Notes', icon: FileText, color: 'text-slate-400' },
                      ].map(section => {
                        const Icon = section.icon;
                        const isExpanded = expandedSections[section.key];
                        return (
                          <div key={section.key} className="border border-slate-800/80 rounded-xl overflow-hidden bg-slate-950/20">
                            <button
                              onClick={() => toggleSection(section.key)}
                              className="w-full px-5 py-4 bg-slate-900/40 hover:bg-slate-900/80 flex justify-between items-center text-sm font-bold text-slate-300 transition-all border-b border-slate-800/20"
                            >
                            ...
                            <AnimatePresence initial={false}>
                              {isExpanded && (
                                ...
                                >
                                  <div className="p-6 text-[15px] md:text-base leading-relaxed text-slate-100 font-medium whitespace-pre-wrap bg-slate-950/70 border-t border-slate-900/60">
                                    {remediationPlaybook[section.key]}
                                  </div>
                                </motion.div>
                              )}
                            </AnimatePresence>
                ...
                >
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <form onSubmit={handleGenerateRca} className="flex flex-col gap-5 bg-slate-950/40 p-7 rounded-xl border border-slate-800/80">
                      <h4 className="text-base font-bold text-slate-300 uppercase tracking-wider border-b border-slate-900 pb-2.5">
                        Forensic Parameters
                      </h4>

                      <div className="flex flex-col gap-2">
                        <label className="text-[12px] font-bold uppercase text-slate-400 font-mono">Incident Identifier</label>
                        <input
                          type="text"
                          required
                          value={rcaForm.incidentId}
                          onChange={(e) => setRcaForm(prev => ({ ...prev, incidentId: e.target.value }))}
                          className="px-4 py-3 bg-slate-950 border border-slate-800 rounded-lg text-[15px] font-mono text-slate-200 focus:outline-none focus:border-cyan-500 transition-all"
                        />
                      </div>

                      <div className="flex flex-col gap-2">
                        <label className="text-[12px] font-bold uppercase text-slate-400 font-mono">Attack Class / Scenario</label>
                        <select
                          value={rcaForm.attackType}
                          onChange={(e) => setRcaForm(prev => ({ ...prev, attackType: e.target.value }))}
                          className="px-4 py-3 bg-slate-950 border border-slate-800 rounded-lg text-[15px] text-slate-200 focus:outline-none focus:border-cyan-500 transition-all"
                        >
                      ...
                      <div className="flex flex-col gap-2">
                        <label className="text-[12px] font-bold uppercase text-slate-400 font-mono">Affected Asset Keys (comma separated)</label>
                        <input
                          type="text"
                          required
                      ...
                      <div className="flex flex-col gap-2">
                        <label className="text-[12px] font-bold uppercase text-slate-400 font-mono">Primary Intrusion Vector</label>
                        <input
                          type="text"
                          required
                      ...
                      <div className="flex flex-col gap-2">
                        <label className="text-[12px] font-bold uppercase text-slate-400 font-mono">Detection Latency Gap</label>
                        <input
                          type="text"
                          required
                    ...
                    {/* Right Column Output */}
                    <div className="bg-slate-950/20 border border-slate-800 rounded-xl p-6 flex flex-col min-h-[400px] relative max-h-[550px] overflow-y-auto custom-scrollbar">
                      {rcaLoading ? (
                        ...
                        <div className="flex flex-col gap-4">
                          {[
                            { title: 'Executive Summary', content: rcaReport.executiveSummary, color: 'text-cyan-400' },
                            { title: 'Intrusion Timeline', content: rcaReport.timeline, color: 'text-rose-400' },
                            { title: 'Root Cause', content: rcaReport.rootCause, color: 'text-amber-400' },
                            { title: 'Contributing Factors', content: rcaReport.contributingFactors, color: 'text-blue-400' },
                            ...
                            { title: 'Prevention & Compliance Measures', content: rcaReport.preventionMeasures, color: 'text-slate-400' },
                          ].map((sec, idx) => (
                            <div key={idx} className="flex flex-col gap-2 text-sm">
                              ...
                              <p className="text-[15px] leading-relaxed text-slate-100 font-medium bg-slate-950/70 p-4 rounded-lg border border-slate-900 whitespace-pre-wrap">
                                {sec.content}
                              </p>
                            </div>
                          ))}
              ...
              {activeTab === 'policy' && (
                ...
                >
                  <div className="flex justify-between items-center border-b border-slate-800 pb-4">
                    <div>
                      <h3 className="text-base font-bold text-white uppercase tracking-wide">
                        MITRE Technique Information Security Policy
                      </h3>
                      <p className="text-xs text-slate-400 mt-1">
                        Formal regulatory compliant policy (Union Bank of India CISO Directive)
                      </p>
                    </div>
                    ...
                    {policyData && (
                      <div className="flex gap-2">
                        ...
                        <button
                          onClick={() => handleDownload(`${selectedCve?.cveId || 'CVE-2026-1043'}_Security_Policy.md`, policyData)}
                          className="px-3.5 py-2 bg-cyan-500/10 hover:bg-cyan-500/20 border border-cyan-500/20 hover:border-cyan-500/40 text-cyan-400 rounded-lg text-xs font-bold uppercase tracking-wider flex items-center gap-1.5 transition-all"
                        >
                          <Download size={14} />
                          Download
                  ...
                  {policyLoading ? (
                    <div className="py-32 flex flex-col items-center justify-center gap-4">
                      ...
                      <div className="text-center">
                        <p className="text-xs text-slate-300 font-bold uppercase tracking-widest animate-pulse">
                          CISO Dashboard: Drafting Regulatory Control
                        </p>
                        <p className="text-[10px] text-slate-500 mt-1">
                          Aligning to RBI Information Security Guidelines, SEBI CSCRF, and ISO 27001 control requirements...
                        </p>
                      </div>
                    </div>
                  ) : policyData ? (
                    <div className="bg-slate-950/70 rounded-xl p-6 border border-slate-800 max-h-[600px] overflow-y-auto custom-scrollbar">
                      <div className="text-[15px] md:text-base leading-relaxed text-slate-100 font-medium whitespace-pre-wrap">
                        {policyData}
                      </div>
                    </div>
                  ) : (
                    <div className="py-24 text-center text-slate-500 flex flex-col items-center gap-2">
                      <span>📄</span>
                      <p className="text-xs">Select a top-risk CVE on the left to draft its MITRE Attack Technique defense policy.</p>
                    </div>
                  )}
                </motion.div>
              )}
